...
def normalizeDF(save=1, dataset=""): #Normalize BIKED Data
    ...
    if save==1:
        scdf.to_csv(Path('Biked_Reference_Data/' + dataset + 'BIKED_normalized.csv')) 
...
def standardizeReOH(df, dataset, intermediates=0):
 ...
 # Can be used to compare against the final BIKED_processed data, as it will have the same parameter space
    ...
    if intermediates!=0:
        df.to_csv(Path("Biked_Reference_Data/"+intermediates+"_reDF.csv"))
    return df
...
def deNormalizeDF(df, dataset="", round=1, intermediates=1):
    ...
    if round==1:
        invdf=invdf.round(6)
    if intermediates!=0:
        invdf.to_csv(Path("Biked_Reference_Data/"+intermediates+"_denorm.csv")) 
...
# we will keep track of the most probable category for each model and categorical feature in a dict with keys containing model/feature pairs
    ...
    dtypedf=pd.read_csv(Path("Biked_Reference_Data/" + dataset + "BIKED_datatypes.csv"), index_col=0).T
    ...
    if intermediates!=0:
        newdf.to_csv(Path(Path("Biked_Reference_Data/"+intermediates+"_deOH.csv")))
    return newdf
...
def loadScaledDF(dataset=""):
    start_time = time.time()
    df=pd.read_csv(Path("Biked_Reference_Data/" + dataset + "BIKED_normalized.csv"), index_col=0) 
    print("Loaded Scaled Dataframe in  %s seconds" % (time.time() - start_time))
    return df
    
def loadVAEGenDF():
    start_time = time.time()
    df=pd.read_csv(Path("Biked_Reference_Data/VAEGendf.csv"), index_col=0)
...
def loadCorrDF(dataset="", metric="pearson"):
    start_time = time.time()
    df=pd.read_csv(Path("Biked_Reference_Data/" + dataset + "-" + metric+ "corrdf.csv"), index_col=0) 
...
def exportCorrDF(fvs=0, method='cosine', dataset=""):
    ...
    filepath=Path('Biked_Reference_Data/' + dataset + "-" + method + 'corrdf.csv')
    if fvs==0:
...
def loadDF():
    start_time = time.time()
    df=pd.read_csv(Path("Biked_Reference_Data/df.csv", index_col=0)) 
    print("Loaded Dataframe in  %s seconds" % (time.time() - start_time))
    return df
    
def loadOHDF():
    start_time = time.time()
    df=pd.read_csv(Path("Biked_Reference_Data/OHdf.csv"), index_col=0)
    print("Loaded One-Hot Dataframe in  %s seconds" % (time.time() - start_time))
    return df

def loadDropDF(dataset=""):
    start_time = time.time()
    df=pd.read_csv(Path("Biked_Reference_Data/" + dataset + "BIKED_reduced.csv"), index_col=0) 
    print("Loaded Reduced Parameter Space Dataframe in  %s seconds" % (time.time() - start_time))
    return df

def loadClassDF(dataset=""): 
    start_time = time.time()
    df=pd.read_csv(Path("Biked_Reference_Data/" + dataset + "classdf.csv"), index_col=0)
    print("Loaded Class  Dataframe in  %s seconds" % (time.time() - start_time))
    return df

def loadProcessedDF(dataset=""):
    start_time = time.time()
    df=pd.read_csv(Path("Biked_Reference_Data/" + dataset + "BIKED_processed.csv"), index_col=0)
...
def processDF(dropdf, intermediates=0, dataset=""):
    ...
    # OHdf.to_csv(Path("Biked_Reference_Data/OHdf.csv"))
...
def getclassdf(dataset=""):
    ...
    classdf.to_csv(Path("Biked_Reference_Data/" + dataset + "classdf.csv"))    
    meddf=pd.DataFrame(index=classdf.index, columns=["medidx"],data=indices)
    meddf.to_csv(Path("Biked_Reference_Data/" + dataset + "meddf.csv"))
...
def imputeNan(df, dataset=""): #Impute missing Values and remove outliers
    ...
    impdf.to_csv(Path('Biked_Reference_Data/' + dataset + 'BIKED_processed.csv')) 
...
def convertOneHot(df, dataset="", save=1):
    ...
    OHdtypes.to_csv("Biked_Reference_Data/" + dataset + "BIKED_processed_datatypes.csv",header=["type"])
    if save==1:
        df.to_csv(Path('Biked_Reference_Data/' + dataset + 'OHdf.csv'))
...
#     empty=pd.Series([Nan]*20, index=df.columns)
    ...
    df.to_csv(Path("Biked_Reference_Data/interpolatedf.csv"))
    return df
